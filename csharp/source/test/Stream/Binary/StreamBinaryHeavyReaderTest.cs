...
/// <summary>
/// <see cref="StreamBinaryHeavyReader" />専用検証クラスです。
/// </summary>
[TestFixture]
public class StreamBinaryHeavyReaderTest {
	...
	/// <summary>
	/// <see cref="StreamBinaryHeavyReader.Read()" />を検証します。
	/// </summary>
	/// <param name="import">検証情報</param>
	[TestCase("")]
	[TestCase("00")]
	[TestCase("000102030405060708090A0B0C0D0E0F")]
	public async Task Success1(string import) {
	...
	/// <summary>
	/// <see cref="StreamBinaryHeavyReader.Read(byte[], int, int)" />を検証します。
	/// </summary>
	/// <param name="import">検証情報</param>
	[TestCase("")]
	[TestCase("00")]
	[TestCase("000102030405060708090A0B0C0D0E0F")]
	public async Task Success2(string import) {
		Assert.That(import.Length % 2, Is.EqualTo(0));
		using var stream = new MemoryStream(ToData(import));
		using var source = new StreamBinaryHeavyReader(stream);
		...
		var result = new StringBuilder();
		while (true) {
			var choose = await source.Read(buffer, 0, buffer.Length);
			if (choose <= 0) {
				break;
			} else {
	...
	public void Failure1() {
		...
		var result = Assert.Throws<ArgumentNullException>(static () => new StreamBinaryHeavyReader(null));
